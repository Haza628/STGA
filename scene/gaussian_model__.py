...
class GaussianModel:
    ...
    def get_scaling(self):
        ...
        if not self.dynamic:
            ...
            ret  = self.scaling_activation(self._scaling[self.gaussian_indices])
    ...
    def get_all_rotation(self):
        ...
        rot = self.rotation_activation(torch.cat((self._rotation[self.gaussian_indices], self._rotation[~self.gaussian_indices])))
        world_rotation_q = quat_xyzw_to_wxyz(quat_product(quat_wxyz_to_xyzw(torch.cat((transform_rotation_q[gaussian_indices], transform_rotation_q[~gaussian_indices]))), quat_wxyz_to_xyzw(rot)))
    ...
    def get_rotation(self):
        ...
        if not self.dynamic:
            ...
            rot = self.rotation_activation(self._rotation[self.gaussian_indices])
            world_rotation_q = quat_xyzw_to_wxyz(quat_product(quat_wxyz_to_xyzw(transform_rotation_q), quat_wxyz_to_xyzw(rot)))
    ...
    def get_all_xyz(self):
        ...
        indices_training = self.training_indices[self.current_keyframe]
        gaussian_indices = torch.isin(indices, indices_training).squeeze()
        world_points = torch.matmul(torch.cat((transform_matrices[gaussian_indices], transform_matrices[~gaussian_indices])), (torch.cat((self._xyz[self.gaussian_indices], self._xyz[~self.gaussian_indices]))).unsqueeze(-1) ).squeeze(-1) + torch.cat((o[gaussian_indices], o[~gaussian_indices]))
    ...
    def get_xyz(self):
        ...
        if not self.dynamic:
            world_points = torch.matmul(transform_matrices, (self._xyz).unsqueeze(-1) ).squeeze(-1) + o
            # save_pts('output/temp.obj', world_points)

        else:


            world_points = torch.matmul(transform_matrices, (self._xyz[self.gaussian_indices]).unsqueeze(-1) ).squeeze(-1) + o
    ...
    def get_features(self):
        ...
        if not self.dynamic:
            ...
            features_dc = self._features_dc[self.gaussian_indices]
            features_rest = self._features_rest[self.gaussian_indices]
    ...
    def get_opacity(self):
        ...
        if not self.dynamic:
            ...
            return self.opacity_activation(self._opacity[self.gaussian_indices])
    ...
    def get_global_params(self):
        ...
        self._global_xyz = torch.matmul(transform_matrices, (self._xyz[~self.gaussian_indices]).unsqueeze(-1) ).squeeze(-1) + o
        self._global_rotation = quat_xyzw_to_wxyz(quat_product(quat_wxyz_to_xyzw(transform_rotation_q), quat_wxyz_to_xyzw(self.rotation_activation(self._rotation[~self.gaussian_indices]))))
        self._global_scaling = self.scaling_activation(self._scaling[~self.gaussian_indices])
        self._global_opacity = self.opacity_activation(self._opacity[~self.gaussian_indices])
        self._global_features = torch.cat((self._features_dc[~self.gaussian_indices], self._features_rest[~self.gaussian_indices]), dim=1)
    ...
    def update_frame(self, keyframe, training_args, dynamic):
        ...
        if dynamic:
            # # self._indices_frozen = self.indices[~gaussian_indices].cuda()
            # self._indices_training = self.indices[:].cuda()

            # self._xyz_training = self._xyz[:].cuda()
            # self._xyz_frozen = torch.zeros((0, self._xyz.shape[1])).cuda()

            # self._rotation_training = self._rotation[:].cuda()
            # self._rotation_frozen = torch.zeros((0, self._rotation.shape[1])).cuda()

            # self._scaling_training = self._scaling[:].cuda()
            # self._scaling_frozen = torch.zeros((0, self._scaling.shape[1])).cuda()

            # self._opacity_training = self._opacity[:].cuda()
            # self._opacity_frozen = torch.zeros((0, self._opacity.shape[1])).cuda()

            # self._features_dc_training = self._features_dc[:].cuda()
            # self._features_dc_frozen = torch.zeros((0, self._features_dc.shape[1], self._features_dc.shape[2])).cuda()

            # self._features_rest_training = self._features_rest[:].cuda()
            # self._features_rest_frozen = torch.zeros((0, self._features_rest.shape[1], self._features_rest.shape[2])).cuda()

            # iii = 200000
            
            # self._indices_training = self.indices[:iii].cuda().long()
            # self._indices_frozen =   self.indices[iii:].cuda().long()

            # self._xyz_training = self._xyz[:iii].cuda()
            # self._xyz_frozen =   self._xyz[iii:].cuda()

            # self._rotation_training = self._rotation[:iii].cuda()
            # self._rotation_frozen =   self._rotation[iii:].cuda()

            # self._scaling_training = self._scaling[:iii].cuda()
            # self._scaling_frozen =   self._scaling[iii:].cuda()

            # self._opacity_training = self._opacity[:iii].cuda()
            # self._opacity_frozen =   self._opacity[iii:].cuda()

            # self._features_dc_training = self._features_dc[:iii].cuda()
            # self._features_dc_frozen =   self._features_dc[iii:].cuda()

            # self._features_rest_training = self._features_rest[:iii].cuda()
            # self._features_rest_frozen =   self._features_rest[iii:].cuda()

            # self._xyz_frozen = nn.Parameter(self._xyz_frozen, requires_grad=False)
            # self._features_dc_frozen = nn.Parameter(self._features_dc_frozen, requires_grad=False)
            # self._features_rest_frozen = nn.Parameter(self._features_rest_frozen, requires_grad=False)
            # self._scaling_frozen = nn.Parameter(self._scaling_frozen, requires_grad=False)
            # self._rotation_frozen = nn.Parameter(self._rotation_frozen, requires_grad=False)
            # self._opacity_frozen = nn.Parameter(self._opacity_frozen, requires_grad=False)

            # # self._xyz_training = nn.Parameter(self._xyz_training, requires_grad=True)
            # # self._features_dc_training = nn.Parameter(self._features_dc_training, requires_grad=True)
            # # self._features_rest_training = nn.Parameter(self._features_rest_training, requires_grad=True)
            # # self._scaling_training = nn.Parameter(self._scaling_training, requires_grad=True)
            # # self._rotation_training = nn.Parameter(self._rotation_training, requires_grad=True)
            # # self._opacity_training = nn.Parameter(self._opacity_training, requires_grad=True)


            indices_training = self.training_indices[self.current_keyframe]

            # self._indices_training = self.indices
            ...
            self._indices_frozen = self.indices[~gaussian_indices].cuda().clone().detach()
            self._indices_training = self.indices[gaussian_indices].cuda()
            self.gaussian_indices = gaussian_indices

            # self._xyz_frozen = self._xyz[~gaussian_indices].cuda().clone().detach()
            # self._xyz_training = self._xyz[gaussian_indices].cuda().clone().detach()

            # self._rotation_frozen = self._rotation[~gaussian_indices].cuda().clone().detach()
            # self._rotation_training = self._rotation[gaussian_indices].cuda().clone().detach()

            # self._scaling_frozen = self._scaling[~gaussian_indices].cuda().clone().detach()
            # self._scaling_training = self._scaling[gaussian_indices].cuda().clone().detach()

            # self._opacity_frozen = self._opacity[~gaussian_indices].cuda().clone().detach()
            # self._opacity_training = self._opacity[gaussian_indices].cuda().clone().detach()

            # self._features_dc_frozen = self._features_dc[~gaussian_indices].cuda().clone().detach()
            # self._features_dc_training = self._features_dc[gaussian_indices].cuda().clone().detach()

            # self._features_rest_frozen = self._features_rest[~gaussian_indices].cuda().clone().detach()
            # self._features_rest_training = self._features_rest[gaussian_indices].cuda().clone().detach()
    ...
    def save_ply(self, path):
        ...
        if not self.dynamic:
            ...
            em_xyz = torch.cat((self._xyz[self.gaussian_indices], self._xyz[~self.gaussian_indices])).detach().cpu().numpy()
            normals = np.zeros_like(xyz)
            f_dc = torch.cat((self._features_dc[self.gaussian_indices], self._features_dc[~self.gaussian_indices])).detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
            f_rest = torch.cat((self._features_rest[self.gaussian_indices], self._features_rest[~self.gaussian_indices])).detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
            opacities = torch.cat((self._opacity[self.gaussian_indices], self._opacity[~self.gaussian_indices])).detach().cpu().numpy()
            rotation = self.get_all_rotation.detach().cpu().numpy()
            em_rotation = torch.cat((self._rotation[self.gaussian_indices], self._rotation[~self.gaussian_indices])).detach().cpu().numpy()
            scale = torch.cat((self._scaling[self.gaussian_indices], self._scaling[~self.gaussian_indices])).detach().cpu().numpy()
            em_scale = torch.cat((self._scaling[self.gaussian_indices], self._scaling[~self.gaussian_indices])).detach().cpu().numpy()
            # can_area = self.canonical_face_area[self.indices].squeeze(-1).detach().cpu().numpy()

            indices_training = self.training_indices[self.current_keyframe]
            gaussian_indices = torch.isin(self.indices, indices_training).squeeze()
            indices = torch.cat((self.indices[gaussian_indices], self.indices[~gaussian_indices])).detach().cpu().numpy()
    ...
    def load_ply(self, path:str, keyframe_meshes:dict, training_indices:dict, spatial_lr_scale:float):
        ...
        canonical_area = np.stack(np.asarray(plydata.elements[0]['can_area']))[..., None]
